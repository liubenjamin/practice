...
public class CF1422B {
    public static void main(String[] args) throws IOException {
        ...
        while(t --> 0) {
            ...
            for(int i = 0; i < n / 2; ++i) {
                for(int j = 0; j < m / 2; ++j) {
                    long sum = a[i][j] + a[I - i][j] + a[i][J - j] + a[I - i][J - j];
                    long avg = sum / 4;
                    sum = abs(a[i][j] - avg) + abs(a[I - i][j] - avg) + abs(a[i][J - j] - avg) + abs(a[I - i][J - j] - avg);
                    ans += sum;
                }
            }
            if(n % 2 == 1) {
                int x = n / 2;
                for(int i = 0; i < m / 2; ++i) {
                    ans += abs(a[x][i] - a[x][m - 1 - i]);
                }
            }
            if(m % 2 == 1) {
                int x = m / 2;
                for(int i = 0; i < n / 2; ++i) {
                    ans += abs(a[i][x] - a[n - 1 - i][x]);
                }
            }
            prln(ans);
        }
