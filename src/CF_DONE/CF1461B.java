...
public class CF1461B {
    public static void main(String[] args) throws IOException {
        int t = ri();
        next: while (t --> 0) {
            int n = rni(), m = ni(), dp[][] = new int[n][m];
            char[][] c = new char[n][m];
            long spruces = 0;
            for (int i = 0; i < n; ++i) {
                c[i] = rcha();
                for (int j = 0; j < m; ++j) {
                    if (c[i][j] == '*')
                        // store max height tree the cell can form
            ...
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    for (int k = 1; k <= 500; ++k) {
                        int x = i + k - 1, y = j + k - 1;
                        if (x >= n || y >= m) break; // out of bounds
                        if (dp[x][y] < 2 * k - 1) break; // insufficient cells
                        ++spruces;
                    }
                }
            }
