...
public class CF450B {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int next = l.get(l.size() - 1) - l.get(l.size() - 2);
        while(true) {
            l.add(next);
            next = l.get(l.size() - 1) - l.get(l.size() - 2);
            if(next == l.get(0) && l.size() > 4) break;
        }
        int i = l.get(floorMod(n % l.size() - 1, l.size()));
        prln(floorMod(i, 1000000007));
