...
public class CF300B {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Integer> [] teams = new List[n+1];
        int[] cnt = new int[n+1];
        for(int i = 1; i <= n; ++i) {
        ...
        for(int i = 1; i <= n; ++i) {
            if(cnt[i] > 3) {
                ...
                sb.append(teams[i].get(0) + " " + teams[i].get(1) + " " + teams[i].get(2) + "\n");
            } else if(cnt[i] == 2) {
                duo.add(i);
            } else if(cnt[i] == 1){
                solo.add(i);
        ...
        if(duo.size() > solo.size()) {
            System.out.println(-1);
            return;
        }
        int p = 0;
        for(int i : duo) {
            sb.append(teams[i].get(0) + " " + teams[i].get(1) + " " + teams[solo.get(p++)].get(0) + "\n");
        }
        while(p < solo.size()) {
            sb.append(solo.get(p++) + " " + solo.get(p++) + " " + solo.get(p++) + "\n");
        }
        prln(sb);
