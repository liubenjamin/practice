...
public class CF1451D {
    public static void main(String[] args) throws IOException {
        // P2 can always stay on line x = y, check if P1 can make legal move
        int t = ri();
        while (t --> 0) {
            int d = rni(), k = ni();
            long dsq = ((long) d) * d;
            int cur = 0;
            while (in(cur + 1, cur + 1, dsq, k))
                ++cur;
            if (in(cur + 1, cur, dsq, k))
                prln("Ashish"); // P1 win
            else
                prln("Utkarsh"); // P2 win
        }
    ...
    public static boolean in(long x, long y, long dsq, long k) {
        return dsq - k * x * k * x - k * y * k * y >= 0;
    }
