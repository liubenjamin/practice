...
public class CF1461C {
    public static void main(String[] args) throws IOException {
        int t = ri();
        next: while (t --> 0) {
            int n = rni(), m = ni(); // len and #exp
            int[] a = ria(n);
            int last = n; // last unsorted position
            while (last > 0 && a[last - 1] == last) --last;
            // if last == 0, already sorted so cannot fail
            double fail_prob = last > 0 ? 1 : 0;
            for (int i = 0; i < m; ++i) {
                int r = rni();
                double p = Double.parseDouble(input.nextToken());
                if (r >= last) {
                    fail_prob *= (1 - p);
                }
            }
