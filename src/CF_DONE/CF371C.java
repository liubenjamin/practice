...
public class CF371C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] recipe = rcha();
        int[] cnt = new int[3];
        int[] n = new int[] { rni(), ni(), ni() };
        int[] p = new int[] { rni(), ni(), ni() };
        long r = rl();
        for(char c : recipe) {
            if(c == 'B') ++cnt[0];
            else if(c == 'S') ++cnt[1];
        ...
        while(L <= R) {
            long mid = L + (R - L) / 2; // burgers to make
            long cost = p[0] * max(0, cnt[0] * mid - n[0]) + p[1] * max(0, cnt[1] * mid - n[1]) + p[2] * max(0, cnt[2] * mid - n[2]); // price to make mid amount of burgers
            if(cost <= r) {
                ans = mid;
                L = mid + 1; // upper half to find max ans
            } else R = mid - 1;
        }
        prln(ans);
