...
public class CF433B {
    ...
    public static void main(String[] args) throws IOException {
        ...
        sum1.set(0, a.get(0));
        for(int i = 1; i < n; ++i) {
            sum1.set(i, sum1.get(i - 1) + a.get(i));
        }
        Collections.sort(a);
        sum2.set(0, a.get(0));
        for(int i = 1; i < n; ++i) {
            sum2.set(i, sum2.get(i - 1) + a.get(i));
        }
        int m = ri();
        while(m --> 0) {
            int type = rni(), l = ni() - 1, r = ni() - 1;
            long ans = 0;
            if(type == 1) {
                long left, right = sum1.get(r);
                left = l == 0 ? 0 : sum1.get(l - 1);
                prln(right - left);
            } else {
                long left, right = sum2.get(r);
                left = l == 0 ? 0 : sum2.get(l - 1);
                prln(right - left);
            }
        }
