...
public class CF479C {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int n = ri(), schedule[][] = new int[n][2];
        for(int i = 0; i < n; ++i) {
            schedule[i][0] = rni();
            schedule[i][1] = ni();
        }
        // comparator lambda 2d array sort
        // sort by 1st col then 2nd col
        sort(schedule, (a, b) -> a[0] == b[0] ? b[1] - a[1] : b[0] - a[0]);
        int max = schedule[0][1]; // when all b-days are equal
        for(int i = 1; i < n; ++i) {
           if(max < schedule[i][0] && max < schedule[i][1]) {
                prln(schedule[0][0]);
                close(); return;
            } else {
               max = (max >= schedule[i][0]) ? schedule[i][0] : schedule[i][1];
            }
        }
        prln(schedule[0][1]);
