...
public class CF1461D {
    public static void main(String[] args) throws IOException {
        int t = ri();
        next: while (t --> 0) {
            int n = rni(), q = ni(), a[] = ria(n);
            sort(a);
            long sum = 0;
            for (int x : a)
                sum += x;
            ...
            slice(a, 0, n - 1, sums);
            for (int i = 0; i < q; ++i) {
                pryn(sums.contains(rl()));
            }
        }
        close();
    }

    static void slice(int[] a, int left, int right, Set<Long> sums) {
        if (a[left] == a[right]) return;
        int mid = (a[left] + a[right]) / 2, split = -1;
        long sum = 0;
        for (int i = left; i <= right; ++i) {
            sum += a[i];
            if (split == -1 && a[i + 1] > mid) {
                sums.add(sum);
                sum = 0;
                split = i;
                slice(a, left, split, sums);
            }
        }
        sums.add(sum);
        slice(a, split + 1, right, sums);
    }
