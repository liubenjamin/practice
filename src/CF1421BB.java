...
public class CF1421BB {
    public static void main(String[] args) throws IOException {
        ...
        while(t --> 0) {
            ...
            int ans = 0;
            boolean x = a[0][1] == a[a.length - 2][a.length - 1];
            boolean y = a[1][0] == a[a.length - 1][a.length - 2];
            if(x || y) {
                if (x) {
                    ++ans;
                }
            ...
            else if(a[0][1] + a[1][0] == 1 && a[a.length - 1][a.length - 2] + a[a.length - 2][a.length - 1] == 1) {
                prln(2);
                prln("1 2");
                prln(a.length - 1 + " " + a.length);
                continue;
            }
            prln(ans);
            if(x) {
