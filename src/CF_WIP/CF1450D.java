    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t --> 0) {
            ...
            for (int k = 1; k <= n ; ++k) {
                ...
                System.out.print(permutation(l) ? 1 : 0);
            }
            System.out.println();
        }
        close();
    }
    static boolean permutation(List<Integer> l) {
        Set set = new HashSet(l);
        if (Collections.max(l) != l.size()) return false;
        if (set.size() == l.size()) return true;
        return false;
    }
