...
public class CF580C_succ {
    ...
    static void dfs(ArrayList<ArrayList<Integer>> g, int[] c, int vert, int cats, int max, int m, boolean[] vis) {
        vis[vert] = true;
        if(c[vert] == 1) ++cats;
        else {
            max = max(max, cats);
        ...
        if(g.get(vert).size() == 1 && vert != 1) {
            max = max(max, cats);
            if(max <= m) ++ans;
        } else {
            for(int i = 0; i < g.get(vert).size(); ++i) {
                int v = g.get(vert).get(i);
                if(!vis[v]) {
                    dfs(g, c, v, cats, max, m, vis);
                }
            }
        }
    }
