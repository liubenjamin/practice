...
public class CF1440C1 {
    public static void main(String[] args) throws IOException {
        int t = ri();
        while (t --> 0) {
            int n = rni(), m = ni(), a[][] = new int[n][m];
            for (int i = 0; i < n; ++i) {
                char[] s = rcha();
                for (int j = 0; j < m; ++j) {
                    a[i][j] = s[j] - '0';
            ...
            for (int i = 0; i < n - 1; ++i) {
                for (int j = 0; j < n - 1; ++j) {
                    if (a[i][j] == 1 && a[i][j + 1] == 1 && a[i + 1][j] == 1 && a[i + 1][j + 1] == 1) {
                        op(new int[] {i, j, i + 1, j, i, j + 1}, a, ans);
                        op(new int[] {i + 1, j + 1, i + 1, j, i, j + 1}, a, ans);
                        op(new int[] {i, j, i + 1, j, i + 1, j + 1}, a, ans);
                        op(new int[] {i, j, i, j + 1, i + 1, j + 1}, a, ans);
                    } else if (a[i][j] == 1 && a[i][j + 1] == 1 && a[i + 1][j] == 1) {
                        op(new int[] {i, j, i, j + 1, i + 1, j}, a, ans);
                    } else if (a[i][j] == 1 && a[i + 1][j + 1] == 1 && a[i + 1][j] == 1) {
                        op(new int[] {i, j, i + 1, j + 1, i + 1, j}, a, ans);
                    } else if (a[i][j] == 1 && a[i][j + 1] == 1 && a[i + 1][j + 1] == 1) {
                        op(new int[] {i, j, i, j + 1, i + 1, j + 1}, a, ans);
                    } else if (a[i][j + 1] == 1 && a[i + 1][j] == 1 && a[i + 1][j + 1] == 1) {
                        op(new int[] {i, j + 1, i + 1, j, i + 1, j + 1}, a, ans);
    ...
    static void op(int[] flips, int[][] a, List<int[]> ans) {
        ans.add(flips);
        a[flips[0]][flips[1]] = 1 - a[flips[0]][flips[1]];
        a[flips[2]][flips[3]] = 1 - a[flips[2]][flips[3]];
        a[flips[4]][flips[5]] = 1 - a[flips[4]][flips[5]];
    }
